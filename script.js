/* =========================================================
   Learn Japanese — App Script (path-tolerant build)
   - Tolerates menifest.json / Lesson.csv / Lesson.pdf
   - Waits for Firebase (whenFBReady) before use
   ========================================================= */
...
(() => {
  ...
  const elPgFeedback = D("#pg-feedback"); // now wired
  ...
  function speakJa(text){ try{ const u=new SpeechSynthesisUtterance(text); u.lang="ja-JP"; speechSynthesis.cancel(); speechSynthesis.speak(u);}catch{} }
  ...
  // Helpers to tolerate your structure
  async function getFirstOk(urls) {
    for (const u of urls) {
      try { const r = await fetch(u, { method: "GET", cache: "no-cache" }); if (r.ok) return u; } catch {}
    }
    return null;
  }
  async function fetchJSONFirst(urls) {
    const u = await getFirstOk(urls);
    if (!u) return null;
    try { return await (await fetch(u, { cache: "no-cache" })).json(); } catch { return null; }
  }
  ...
  // Probe common assets; tolerate menifest.json + Lesson.pdf
  ...
  async function loadVocabFileList(level, lesson) {
    ...
    // No manifest? probe common file names you use
    const candidates = [
      "Lesson.csv",
      `${lesson}.csv`,                // Lesson-01.csv
      `${lesson.toLowerCase()}.csv`,  // lesson-01.csv
    ];
    const existing = [];
  ...
  window.showLearnRomaji = () => toast("Romaji: (not available in CSV) — Add your own in note.");
  window.learnNoteSaveNow = async () => {
    const w = App.deckFiltered[App.qIndex]; if (!w) return;
    const key = keyForWord(w);
    try { const fb = await whenFBReady(); await fb.setNote(key, elLearnNoteText.value || ""); elLearnNoteStatus.textContent = "Saved ✓"; toast("Note saved"); }
  ...
  window.startPractice = async (mode) => {
    ...
    App.qIndex = 0; App.stats = { right: 0, wrong: 0, skipped: 0 }; updateScorePanel();
    hideAllPractice(); elQuestionBox.innerHTML = ""; elOptions.innerHTML = ""; elExtraInfo.textContent = "";
  ...
  function onPickOption(btn, isCorrect) {
    ...
    if (isCorrect) { btn.classList.add("is-correct"); App.stats.right++; incrementPoints(1); }
  ...
  function renderDualQuestion(w) {
    ...
    else { prompt = `${w.hira} · ${w.en}`; correctLeft = w.kanji; } // he2k
    elQuestionBox.textContent = prompt;
    ...
    function finalizePick() {
      if (mode === "he2k") {
        ...
        const ok = pickedLeft === correctLeft; markDualResults(ok, ok);
      } else {
        if (pickedLeft == null || pickedRight == null) return;
        const ok = (pickedLeft === correctLeft) && (pickedRight === correctRight); markDualResults(ok, ok);
      }
    }
    function markDualResults(isCorrectLeft, isCorrectRight) {
      if (mode === "he2k") { if (isCorrectLeft) { App.stats.right++; incrementPoints(1); } else { App.stats.wrong++; recordMistake(w); } }
      else { if (isCorrectLeft && isCorrectRight) { App.stats.right++; incrementPoints(1); } else { App.stats.wrong++; recordMistake(w); } }
      updateScorePanel(); setTimeout(() => { App.qIndex++; updateDeckProgress(); renderQuestion(); }, 350);
    }

    for (const val of pickSetLeft) {
      const b = document.createElement("button"); b.textContent = val;
      b.addEventListener("click", () => { A(".dual-grid > :first-child button").forEach(x => x.disabled = true);
        b.classList.add(val === correctLeft ? "is-correct" : "is-wrong"); pickedLeft = val; finalizePick(); });
      leftCol.appendChild(b);
    }
    for (const val of pickSetRight) {
      const b = document.createElement("button"); b.textContent = val;
      b.addEventListener("click", () => { A(".dual-grid > :last-child button").forEach(x => x.disabled = true);
        b.classList.add(val === (correctRight || w.en) ? "is-correct" : "is-wrong"); pickedRight = val; finalizePick(); });
  ...
  window.writeSubmit = () => {
    ...
    if (normalizeJa(ans) === normalizeJa(w.hira)) {
      ...
      App.stats.right++; incrementPoints(1); App.write.idx++; updateScorePanel(); setTimeout(renderWriteCard, 300);
    } else {
  ...
  window.writeShowDetails = () => { const i = App.write.order[App.write.idx] ?? -1; const w = App.deckFiltered[i]; if (w) toast(`Hint: ${w.en} → ${w.hira}`); };
  ...
  window.makeSubmit = () => { const text = (elMakeInput.value || "").trim(); if (!text) return; App.stats.right++; incrementPoints(1); elMakeFeedback.textContent = "✓ Saved."; };
  ...
  function setMistakes(arr){ try { localStorage.setItem(MKEY, JSON.stringify(arr)); } catch {} }
  ...
  async function setupListPractice(source, mode) {
    ...
    if (mode === "learn") {
      App.deckFiltered = shuffle(deck); App.qIndex = 0; hideAllPractice(); elLearn.classList.remove("hidden"); renderLearnCard();
    } else if (mode === "write") {
      App.deckFiltered = shuffle(deck); App.write.order = App.deckFiltered.map((_, i) => i); App.write.idx = 0; hideAllPractice(); elWrite.classList.remove("hidden"); renderWriteCard();
    } else {
      App.deckFiltered = shuffle(deck); App.qIndex = 0; hideAllPractice(); D("#practice").classList.remove("hidden"); updateDeckProgress(); renderQuestion();
    }
  }
  ...
  window.markCurrentWord = async () => {
    ...
    try { const fb = await whenFBReady(); await fb.markWord(id, { kanji: w.kanji, hira: w.hira, en: w.en, front: w.hira, back: w.en }); toast("Marked ✓"); }
  ...
  window.addEventListener("beforeunload", () => {
    try { /* best effort; Firestore commit before unload is not guaranteed */ } catch {}
  });
